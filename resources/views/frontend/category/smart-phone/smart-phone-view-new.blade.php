...
@section('title', 'AMIGO - Smart Phone')
@section('head')
...
<main class="main amigo-col">
    ...
    @php
        ...
        foreach ($data as $key => $item) {
            ...
            $highlighted_spec = $item->highlighted_spec;

            if (isset($item->smart_phone[0])) {
                $over_view_image_large = $item->smart_phone[0]->over_view_image_large;
            }

            // if($item->feature_phone[0]->over_view_image_large!=null){
            //     echo $item->feature_phone[0]->over_view_image_large;
    ...
    <div class=" pt-1 pb-1" style="background-color:#e8e8e8!important">
        ...
        <div class="container overview">
            <ul class="nav nav-pills ">
                ...
                <li class="pr-3" style="margin-top:3px;"><a style="font-size: 1.2em;" id="in_the_box_button"
                        href="#">In
                        The
                        Box</a>
                </li>
        ...
        <div class="tab-pane fade show active" style="padding: 0rem 0rem!important;" id="overview">
            ...
            @if (count($over_view_images) > 0)
                ...
                @foreach ($over_view_images as $item)
                    ...
                    @endphp
                    @if ($upper_image != 'null')
                        <img src="{{ asset($upper_image) }}" class="img-responsive center-block d-block mx-auto"
                            alt="Responsive image">
                        {{-- <img src="{{ asset($upper_image) }}" style=" display: flex; justify-content: center;" class="img-fluid" alt="Responsive image"> --}}
                    @endif
                @endforeach
            @endif
            <div id="highlighted_spec">
                <img src="{{ asset($highlighted_spec) }}" class="img-responsive center-block d-block mx-auto"
                    alt="Responsive image">
            </div>
            <div id="over_view_image_div">
                <div class=" center-block" style=" background-color: #FFFFFF;">
                    <img src=" {{ asset($over_view_image_large) }}" id="colour_thumbnail_image"
                        class="img-responsive center-block d-block mx-auto" alt="Sample Image"
                    ...
                    @if (count($data) > 0)
                        @foreach ($data as $item)
                            ...
                            <div class="center item-hover-variant-image "
                                style=" display: flex; justify-content:center;">
                                @foreach ($item->smart_phone as $image)
                                    <a href="javascript:void(0)" class="active" id="ab-{{ $i }}">
                                        <img src="{{ asset($image->colour_thumbnail) }}"
                                            ...
                                            style="width: 40px;height:40px; margin-left:5px!important"
                                            style="z-index: 3;background-color: #FFFFFF;    position: relative;">
                                    </a>
                                    @php
                                        $i++;
                                    @endphp
            ...
            @if (count($over_view_images) > 0)
                @foreach ($over_view_images as $item)
                    ...
                    @endphp
                    @if ($lower_image != 'null')
                        <img src="{{ asset($lower_image) }}" class="img-responsive center-block d-block mx-auto"
                            alt="Responsive image">
                    @endif
                @endforeach
            @endif
    ...
    <div id="in_the_box_div">
        <img src="{{ asset($in_box_image) }}" class="img-responsive center-block d-block mx-auto"
            alt="Responsive image">
    </div>
    ...
    <div class="tab-pane fade" id="spec" style="font-weight: 600 !important;color:black">
        <div class="product-desc-content" id="product-desc-tab">
            ...
            <div class="container">
                <div class="col-12 text-center description" id="specification">
...
<script>
    $("#product-zoom").elevateZoom({
        ...
        $(document).on('click', '.item-hover-variant-image a', function() {
            ...
            var over_view_image = image.replace('http://127.0.0.1:8000/uploads/product/smartphone/variant/', '');
        ...
        $(document).ready(function() {
            ...
            $.ajax({
                type: "GET",
                url: "/smart-phone-view-specification/" + id,
                dataType: "json",
                success: function(response) {
                    // console.log(response.data)
                    $("#specification").html("");
                    ...
                    $.each(response.data, function(key, item) {
                        ...
                        $("#specification").append(item.description);
